...
3. General Instructions
    ...
    2. Project Structure
    3. Available Tools
    4. Key Pydantic Concepts
4. Exercise 0: Space Station Data
    1. Background
    2. Requirements
5. Exercise 1: Alien Contact Logs
    1. Background
    2. Requirements
6. Exercise 2: Space Crew Management
    1. Background
    2. Requirements
...
Welcome to the **Cosmic Data Observatory**, the galaxy’s premier data processing facility! As a junior Data Engineer, you’ve been assigned to validate incoming data streams from various space missions, alien contact reports, and station monitoring systems. Your mission: Learn **Pydantic**; Python’s most powerful data validation library; through hands-on cosmic scenarios. You’ll progress from basic models to complex validation rules, ensuring data integrity across the known universe.
...
- You must use which **Pydantic 2.x** for every exercise. (It must be installed via pip). Only other modules will be listed in each exercise’s **Allowed** section
...
**BaseModel** The foundation of all Pydantic models. Inherit from `BaseModel` to create validated data classes. **Field** Use `Field(...)` to add validation constraints, descriptions, and default values to model attributes. **model_validator** Use the `@model_validator(mode=’after’)` decorator for custom validation logic that runs after Pydantic’s built-in validation. This replaces the deprecated `@validator` decorator from Pydantic v1.
...
**SpaceStation Model** Create a Pydantic model with these validated fields:
...
**Demonstration Function** Include a `main()` function that:
...
**ContactType Enum** Define contact types: `radio`, `visual`, `physical`, `telepathic`

**AlienContact Model** Create a Pydantic model with these fields:
...
**Demonstration Function** Show valid and invalid contact reports with clear error messages. Expected Output Example:
...
**Rank Enum** Define crew ranks: `cadet`, `officer`, `lieutenant`, `captain`, `commander`

**CrewMember Model** Individual crew member with these fields:
...
**SpaceMission Model** Mission with crew list and these fields:
...
**Mission Validation Rules** Implement @model_validator(mode=’after’) with these safety requirements: 
...
**Demonstration Function** Show a valid mission with crew details and an invalid mission that fails validation. Expected Output Example:
